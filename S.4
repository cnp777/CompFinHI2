...
theta_hat_digital_half = solve(0.5*t(X_BS)%*%X_BS+0.5*t(Y_BS)%*%Y_BS)%*%(0.5*t(X_BS)%*%(samples_BS$DigitalT)+0.5*t(Y_BS)%*%(samples_BS$Digital_delta_LRM))
theta_hat_digital_delta = solve(t(YY_BS)%*%YY_BS)%*%t(YY_BS)%*%(samples_BS$Digital_delta_LRM)
...
ggplot(df_BS, aes(x=S0_BS))  +
  geom_function(fun=delta_regression, args = list(theta_hat_digital, p) , colour = "red") + 
...
# FIGURE 3 
png("S4Figure3.png")
ggplot(df_BS, aes(x=S0_BS, y=Digital_delta_LRM)) + geom_point(color = 'grey', shape = 1) + 
  theme_classic() + ylab('Digital Call Delta') + xlab("Initial stock price") + 
  ggtitle("Estimated Digital Call Deltas in the Black-Scholes Model") + theme(plot.title = element_text(hjust = 0.5, face = "bold")) +
  geom_function(fun=trueDelta_Digital, args = list(t=0) , colour = "black") + 
  geom_function(fun=delta_regression, args = list(theta_hat_digital_price, p) , colour = "blue", xlim = c(0.6,1.75)) +
  geom_function(fun=delta_regression, args = list(theta_hat=theta_hat_digital_half, p=p), colour = "red", xlim = c(0.6,1.75)) + 
  geom_function(fun=delta_regression_deltaonly, args = list(theta_hat=theta_hat_digital_delta, p=p), colour = "orange",xlim = c(0.6,1.75)) +
  scale_x_continuous(expand = c(0, 0), limits = c(0.42, 1.8), sec.axis = dup_axis(breaks=NULL, name=NULL)) + 
  scale_y_continuous(sec.axis = dup_axis(breaks=NULL,name=NULL), limits = c(-0.2, 2)) +
  annotate(geom="text", x=0.5, y=2, label= "Grey o's: Simulated Deltas", color="grey", hjust = 0, size=3) +
  annotate(geom="text", x=0.5, y=1.9, label= "Black: True Delta", color="black", hjust = 0, size=3) +
  annotate(geom="text", x=0.5, y=1.85, label= "Blue: Price only-regression", color="blue", hjust = 0, size=3) +
  annotate(geom="text", x=0.5, y=1.8, label= "Red: Half/half-regression", color="red", hjust = 0, size=3) +
  annotate(geom="text", x=0.5, y=1.75, label= "Orange: Delta only-regression", color="orange", hjust = 0, size=3) 
dev.off()
...
for (q in 1:length(polydeg)){
  for (sim in 1:length(simulations)){
    ...
    DigitalT = rep(NA, n)           # instead of C  <- rep(NA, n)
    Digital_delta_LRM = rep(NA, n)  # instead of D  <- rep(NA, n)
    ...
    for (t in 0:51){
      ...
      thetaHat_half[t+1,]  <- solve(w*t(X)%*%X+(1-w)*t(Y)%*%Y)%*%(w*t(X)%*%DigitalT+(1-w)*t(Y)%*%Digital_delta_LRM)
...
### Black Scholes model: True Values for the Digital Call Option
F <- function(x,t){
